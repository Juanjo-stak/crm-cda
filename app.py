...
with tabs[0]:
    ...
    seleccion = st.sidebar.selectbox("Seleccionar base",[x[0] for x in bases])
    ...
    for p in ["fecha_renovacion","fecha","vencimiento","fecha vencimiento"]:
    ...
    for i,row in df_filtrado.iterrows():
        ...
        c1.write(f"**{row.get('Placa','')}**")
        c1.write(row.get("Cliente",""))

        c2.write(row["Fecha_Renovacion"].date())

        estado=c3.selectbox("Estado",estados,index=estados.index(row["Estado"]),key=f"estado_{i}")
        ...
        url=link_whatsapp(row.get("Cliente",""),
                          row.get("Placa",""),
                          row.get("Telefono",""),
        ...
        if url:
            c4.markdown(f'<a href="{url}" target="_blank"><button style="width:100%;padding:10px;background:#25D366;color:white;border:none;border-radius:8px;">📲 WhatsApp</button></a>',unsafe_allow_html=True)

        telefono=str(row.get("Telefono","")).replace(".0","")
        if telefono:
            c4.markdown(f'<a href="tel:+57{telefono}"><button style="width:100%;padding:8px;background:#1f77b4;color:white;border:none;border-radius:8px;">📞 Llamar</button></a>',unsafe_allow_html=True)
